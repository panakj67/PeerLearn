    ...
    <div onClick={() => dispatch(hideLogin())} className='fixed inset-0 z-30 flex items-center justify-center bg-black/40 px-4'>
      <form onSubmit={onSubmitHandler} onClick={(e) => e.stopPropagation()} className="w-full max-w-[560px] rounded-xl bg-[#ececec] px-6 py-8 shadow-2xl sm:px-10">
        <div className='grid grid-cols-2 gap-4'>
          <button type='button' onClick={() => setMode('login')} className={`h-14 cursor-pointer rounded-md text-lg font-semibold transition ${mode === 'login' ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-300 text-gray-600'}`}>
            LOGIN
          </button>
          <button type='button' onClick={() => setMode('register')} className={`h-14 cursor-pointer rounded-md text-lg font-semibold transition ${mode === 'register' ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-300 text-gray-600'}`}>
            REGISTER
          </button>
        </div>
        ...
        <div className='mt-4 flex justify-center'>
          {googleClientId ? (
            ...
            <button type='button' className='inline-flex cursor-pointer items-center gap-2 rounded-full border border-gray-300 bg-white px-4 py-2 text-sm text-gray-600'>
        ...
        {mode === 'login' && (
          <div className='mt-4 flex items-center justify-between text-sm'>
            <label className='flex items-center gap-2 text-gray-700'>
              <input type='checkbox' defaultChecked className='h-4 w-4 cursor-pointer accent-blue-600' /> Remember me
            </label>
            <button type='button' onClick={() => setMode('forgot')} className='cursor-pointer text-blue-600 hover:underline'>Forgot password?</button>
          </div>
        )}

        {mode === 'verifyOtp' && (
          <div className='mt-3 text-right'>
            <button type='button' onClick={resendOtpHandler} className='cursor-pointer text-sm text-blue-600 hover:underline'>Resend OTP</button>
          </div>
        )}

        {authLoading && (
          <p className='mt-3 text-sm font-medium text-blue-600'>{loadingText}</p>
        )}
        ...
        <button disabled={authLoading} className='mt-6 h-14 w-full cursor-pointer rounded-md bg-blue-600 text-lg font-semibold text-white shadow-lg transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-70'>
          {authLoading ? loadingText : mode === 'register' ? 'SIGN UP' : mode === 'verifyOtp' ? 'VERIFY OTP' : mode === 'forgot' ? 'SEND RESET EMAIL' : mode === 'reset' ? 'RESET PASSWORD' : 'SIGN IN'}
        </button>

        <div className='mt-6 text-center text-gray-700'>
          {primaryModes.includes(mode) ? (
            mode === 'login' ? (
              <p>Not a member? <button type='button' onClick={() => setMode('register')} className='cursor-pointer text-blue-600 hover:underline'>Register</button></p>
            ) : (
              <p>Already have account? <button type='button' onClick={() => setMode('login')} className='cursor-pointer text-blue-600 hover:underline'>Login</button></p>
            )
          ) : (
            <button type='button' onClick={() => setMode('login')} className='cursor-pointer text-blue-600 hover:underline'>Back to login</button>
          )}
        </div>
      </form>
    </div>
